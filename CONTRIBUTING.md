...
Tests are run with Maven Surefire. Tests are organised in the `src/test` folder of each module, and follow the standard Maven naming convention (classes named `*Test.java` or `*Tests.java` are included in the test suite, while classes named `*IntegrationTests.java` are not included by default, but can be run separately).
...
- Run full CI-like build + tests + packaging with MySQL configuration:

```bash
mvn -Dspring.profiles.active=mysql-test package install
```

- Run tests for one module only (and build required dependencies):

```bash
mvn -pl iam-login-service -am test
```

- Run one test class:

```bash
mvn -pl iam-login-service -am -Dtest=ClientRegistrationAPIIntegrationTests test
```

- Run one test method:

```bash
mvn -pl iam-login-service -am -Dtest=ClientRegistrationAPIIntegrationTests#testMethodName test
```

- Run only integration-style tests (by naming convention):

```bash
mvn -pl iam-login-service -am -Dtest='*IntegrationTests' test
```

- Run only non-integration tests (exclude `*IntegrationTests`):

```bash
mvn -pl iam-login-service -am -Dtest='*Test,*Tests,!*IntegrationTests' test
```

- Only output failed tests:

```bash
mvn -pl iam-login-service -am -Dsurefire.showOutput=only-on-failure test
```

- Only output failed tests and their stack traces:

```bash
mvn -pl iam-login-service -am -Dsurefire.showOutput=only-on-failure -Dsurefire.printSummary=true test
``` 

For more options, please refer to the [Maven Surefire documentation](https://maven.apache.org/surefire/maven-surefire-plugin/examples/single-test.html).

Database/profile setup:

- Default local test runs mostly use in-memory configuration (H2 and test-specific Spring profiles declared in test classes).
- To run the suite against a local MySQL instance, use `mysql-test` and set env vars as needed:
  - `IAM_DB_HOST` (default `dev.local.io`)
  - `IAM_DB_PORT` (default `3306`)
  - `IAM_DB_NAME` (default `iam`)
  - `IAM_DB_USERNAME` (default `iam`)
  - `IAM_DB_PASSWORD` (default `pwd`)
- If you prefer containerized MySQL for tests, use:

```bash
mvn -Dspring.profiles.active=mysql-test-tc test
```

Reports are generated as below:
