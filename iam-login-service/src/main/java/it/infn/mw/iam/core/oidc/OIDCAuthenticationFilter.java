...
import it.infn.mw.iam.core.jwt.signer.JWKSetCacheService;
...
public class OIDCAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
  ...
  public Authentication attemptAuthentication(HttpServletRequest request,
    ...
    if (!Strings.isNullOrEmpty(request.getParameter("error"))) {
      ...
      return null; // no auth, response redirected to the server's Auth Endpoint (or possibly to the
                   // account chooser)
    }
  ...
  protected Authentication handleAuthorizationCodeResponse(HttpServletRequest request,
    ...
    if (httpClient == null) {
      httpClient = HttpClientBuilder.create()
        .useSystemProperties()
        .setDefaultRequestConfig(RequestConfig.custom().setSocketTimeout(httpSocketTimeout).build())
        .build();
    }
    ...
    if (SECRET_BASIC.equals(clientConfig.getTokenEndpointAuthMethod())) {
      ...
      if (SECRET_JWT.equals(clientConfig.getTokenEndpointAuthMethod())
        ...
        if (SECRET_JWT.equals(clientConfig.getTokenEndpointAuthMethod())
            && (JWSAlgorithm.HS256.equals(alg) || JWSAlgorithm.HS384.equals(alg)
        ...
        JWSHeader header = new JWSHeader(alg, null, null, null, null, null, null, null, null, null,
            signer.getDefaultSignerKeyId(), null, null);
      ...
      try {
        ...
        if (idClaims.getIssuer() == null) {
          ...
          throw new AuthenticationServiceException("Issuers do not match, expected "
              + serverConfig.getIssuer() + " got " + idClaims.getIssuer());
        }
        ...
        if (idClaims.getAudience() == null) {
          ...
          throw new AuthenticationServiceException("Audience does not match, expected "
              + clientConfig.getClientId() + " got " + idClaims.getAudience());
        }
        ...
        if (!nonce.equals(storedNonce)) {
          logger
            .error("Possible replay attack detected! The comparison of the nonce in the returned "
        ...
        PendingOIDCAuthenticationToken token =
            new PendingOIDCAuthenticationToken(idClaims.getSubject(), idClaims.getIssuer(),
                serverConfig, idToken, accessTokenValue, refreshTokenValue);
   ...
   *
   * @param request The request from which to extract parameters and handle the error
  ...
  /**
   * Handle a successful authentication event. If the issuer service sets a target URL, we'll go to
   * that. Otherwise we'll let the superclass handle it for us with the configured behavior.
   */
