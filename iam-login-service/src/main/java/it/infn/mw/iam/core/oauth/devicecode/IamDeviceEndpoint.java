...
import static it.infn.mw.iam.core.oauth.IamOAuthRequestParameters.APPROVED_SITE;
...
@Controller
public class IamDeviceEndpoint {

  public static final String URL = "/devicecode";

  public static final Logger logger = LoggerFactory.getLogger(IamDeviceEndpoint.class);
  ...
  public IamDeviceEndpoint(IamClientRepository clientRepository, SystemScopeService scopeService,
      IamProperties config, DeviceCodeService deviceCodeService,
      OAuth2RequestFactory oAuth2RequestFactory, UserApprovalHandler iamUserApprovalHandler,
      IamUserApprovalUtils userApprovalUtils, AuthenticationHolderService authHolderService,
  ...
  public String requestDeviceCode(@RequestParam("client_id") String clientId,
    ...
    Collection<String> authorizedGrantTypes = client.get().getAuthorizedGrantTypes();
    if (authorizedGrantTypes != null && !authorizedGrantTypes.isEmpty()
        && !authorizedGrantTypes.contains(IamDeviceCodeTokenGranter.GRANT_TYPE)) {
      model.put(HttpCodeView.CODE, HttpStatus.UNAUTHORIZED);
    ...
    try {
      ...
      if (config.getDeviceCode().getAllowCompleteVerificationUri()) {
  ...
  public String requestUserCode(
    ...
    if (!config.getDeviceCode().getAllowCompleteVerificationUri() || userCode == null) {
  ...
  public String readUserCode(@RequestParam("user_code") String userCode, ModelMap model,
    ...
    DeviceCode dc = deviceCodeService.findByUserCode(userCode);
...
//  }
  ...
  private void approveDevice(DeviceCode dc, OAuth2Authentication o2Auth) {
    ...
    dc.approve();
    AuthenticationHolderEntity authHolder = new AuthenticationHolderEntity(dc.getClient(), o2Auth);
    authHolder.addDeviceCode(dc);
    dc.setAuthenticationHolder(authHolder);
    authHolderService.save(authHolder);
  }
