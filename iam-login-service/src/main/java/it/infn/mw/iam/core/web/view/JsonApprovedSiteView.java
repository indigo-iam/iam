...
import it.infn.mw.iam.persistence.model.serializer.ApprovedSiteSerializer;
...
public class JsonApprovedSiteView extends AbstractView {
  ...
  private Gson gson =
      new GsonBuilder().registerTypeAdapter(ApprovedSite.class, new ApprovedSiteSerializer())
    ...
    .registerTypeAdapter(OAuth2AccessTokenEntity.class,
        new JsonSerializer<OAuth2AccessTokenEntity>() {
          @Override
    ...
    .registerTypeAdapter(WhitelistedSite.class, new JsonSerializer<WhitelistedSite>() {
      @Override
      public JsonElement serialize(WhitelistedSite src, Type typeOfSrc,
          JsonSerializationContext context) {
        return new JsonPrimitive(src.getId());
      }
    })
