...
public class TransientNotificationFactory implements NotificationFactory {
  ...
  @Override
  public IamEmailNotification createExpirationWarningMessage(IamAccount account,
    ...
    model.put(RECIPIENT_FIELD, recipient);
    model.put(ORGANISATION_NAME, organisationName);
    model.put("daysUntilSuspension", daysUntilSuspension);

    String subject = "Account expiration warning";

    IamEmailNotification notification = createMessage("accountExpirationWarning.ftl", model,
        IamNotificationType.ACCOUNT_EXPIRATION_WARNING, subject,
        asList(account.getUserInfo().getEmail()));

    LOG.debug("Created expiration warning message for account {}",
        account.getUserInfo().getName());
