...
(function() {
    ...
    function RejectRequest($uibModalInstance, CertLinkRequestsService, toaster, request, userFullName) {
        var self = this;
        ...
        self.reject = function() {
            CertLinkRequestsService.rejectRequest(self.request, self.motivation).then(function(r) {
    ...
    function CertLinkRequestsController($scope, $rootScope, $uibModal, CertLinkRequestsService, $filter, filterFilter, toaster) {
        var self = this;
        self.loaded = false;
        self.filter = "";
        self.filtered = [];
        ...
        function updateRootScopeCounters(res) {
            $rootScope.pendingCertLinkRequests(res);
        }
        ...
        function loadRequests(page) {
            return CertLinkRequestsService.getCertLinkRequests({ status: 'PENDING', startIndex: page }).then(loadSuccess, errorHandler);
        }
        ...
        function filterRequests() {
            self.filtered = filterFilter(self.requests, function(request) {
                ...
                if (request.subjectDn.toLowerCase().indexOf(query) != -1) {
                    return true;
                }
                if (request.issuerDn.toLowerCase().indexOf(query) != -1) {
                    return true;
                }
        ...
        self.approve = function(req) {
            self.busy = true;
            return CertLinkRequestsService.approveRequest(req).then(approveSuccess, decisionErrorHandler);
        };
    ...
    angular
        .module('dashboardApp')
        .component('certLinkRequests', certLinkRequests());

    function certLinkRequests() {
        return {
            bindings: {
                parentCb: '&'
            },
            templateUrl: '/resources/iam/apps/dashboard-app/components/requests/certificate/requests.cert.component.html',
            controller: ['$scope', '$rootScope', '$uibModal', 'CertLinkRequestsService', '$filter', 'filterFilter', 'toaster', CertLinkRequestsController],
            controllerAs: '$ctrl'
        };
    }
