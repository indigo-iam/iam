...
class ServiceAccountNotificationTests {
  ...
  void testEmailSentForSettingServiceAccount() throws Exception {
    ...
    mvc.perform(patch("/scim/Users/{id}", testUser.getUuid())
      ...
      .contentType("application/scim+json;charset=UTF-8")).andExpect(status().isNoContent());
    List<IamEmailNotification> notifications =
        notificationRepo.findByNotificationType(IamNotificationType.SET_SERVICE_ACCOUNT);

    assertEquals(1, notifications.size());
    assertEquals("[indigo-dc IAM] Account set as service account",
  ...
  @Test
  @WithMockUser(username = "admin", roles = {"ADMIN", "USER"})
  void testEmailSentForRevokingServiceAccount() throws Exception {
    IamAccount testUser = iamAccountRepo.findByUsername("test")
      .orElseThrow(() -> new AssertionError("Expected test user not found"));
    ...
    mvc.perform(patch("/scim/Users/{id}", testUser.getUuid())
      ...
      .contentType("application/scim+json;charset=UTF-8")).andExpect(status().isNoContent());
    List<IamEmailNotification> notifications =
        notificationRepo.findByNotificationType(IamNotificationType.REVOKE_SERVICE_ACCOUNT);
    ...
    assertThat(message.getSubject(),
        equalTo("[indigo-dc IAM] Account's service account status revoked"));
