...
class AuthenticationTimeStamperTests {
  ...
  void setsAuthenticationTimestampOnSession() throws Exception {
    ...
    verify(session).setAttribute(eq(AuthenticationTimeStamper.AUTH_TIMESTAMP),
        argThat(value -> value instanceof java.util.Date));

    verify(session, never()).setAttribute(eq(ConnectRequestParameters.PROMPTED), any());
    verify(session, never()).removeAttribute(ConnectRequestParameters.PROMPT_REQUESTED);
  }
  ...
  void marksPromptedWhenPromptWasRequested() throws Exception {
    ...
    when(request.getSession()).thenReturn(session);
    when(authentication.getName()).thenReturn("test-user");
    when(session.getAttribute(ConnectRequestParameters.PROMPT_REQUESTED)).thenReturn(Boolean.TRUE);

    stamper.onAuthenticationSuccess(request, response, authentication);

    verify(session).setAttribute(ConnectRequestParameters.PROMPTED, Boolean.TRUE);
    verify(session).removeAttribute(ConnectRequestParameters.PROMPT_REQUESTED);
  }
}
