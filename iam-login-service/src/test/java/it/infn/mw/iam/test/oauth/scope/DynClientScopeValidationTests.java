...
class DynClientScopeValidationTests {
  ...
  void setup() {
    ...
    lenient().when(scopeService.getRestricted()).thenReturn(emptySet());
    lenient().when(registry.findMatcherForScope("restricted"))
      .thenReturn(stringEqualsMatcher("restricted"));
    lenient().when(clientService.generateClientSecret(Mockito.any()))
      .thenAnswer(i -> i.getArguments()[0]);
  }

  @Test
  void noFilterTest() throws ValidationException {
  ...
  @Test
  void staticRestrictedScopeFilterTest() throws ValidationException {

    client.setScope(newHashSet("openid", "profile", "restricted"));
    lenient().when(scopeService.getRestricted())
      .thenReturn(Sets.newHashSet(new SystemScope("restricted")));
    client = clientValidationService.validateClient(client);
    assertThat(client.getScope(), hasSize(2));
    assertThat(client.getScope(), hasItem("openid"));
    assertThat(client.getScope(), hasItem("profile"));
  ...
  @Test
  void staticStructuredScopeFilterTest() throws ValidationException {

    client.setScope(newHashSet("openid", "profile", "read", "read:/", "read:/sub/path"));
    lenient().when(scopeService.getRestricted()).thenReturn(newHashSet(new SystemScope("read:/")));
    lenient().when(registry.findMatcherForScope("read:/"))
      .thenReturn(structuredPathMatcher("read", "/"));
    client = clientValidationService.validateClient(client);
