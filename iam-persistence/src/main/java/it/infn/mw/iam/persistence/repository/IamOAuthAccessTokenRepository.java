...
public interface IamOAuthAccessTokenRepository
  ...
  @Query("select t from OAuth2AccessTokenEntity t "
      ...
      + "and ('registration-token' member of t.scope)")
  Optional<OAuth2AccessTokenEntity> findRegistrationToken(@Param("clientId") Long id);

  @Query("select t from OAuth2AccessTokenEntity t "
      + "where t.client.id = :clientId "
      + "and ('resource-token' member of t.scope)")
  Optional<OAuth2AccessTokenEntity> findResourceToken(@Param("clientId") Long id);
  ...
  @Query("select t from OAuth2AccessTokenEntity t "
    + "where t.authenticationHolder.client.clientId = :clientId "
    + "and 'resource-token' not member of t.scope "
    + "and 'registration-token' not member of t.scope "
    + "and t.expiration is NOT NULL "
    + "and t.expiration > :timestamp "
  ...
  @Query("select t from OAuth2AccessTokenEntity t "
    ...
    + "and 'resource-token' not member of t.scope "
    + "and 'registration-token' not member of t.scope "
    + "and t.authenticationHolder.client.clientId = :clientId "
    + "and t.expiration is NOT NULL "
    + "and t.expiration > :timestamp "
    + "order by t.expiration")
  ...
  @Query("select t from OAuth2AccessTokenEntity t where t.approvedSite.id = :approvedSiteId")
  List<OAuth2AccessTokenEntity> findByApprovedSiteId(@Param("approvedSiteId") Long approvedSiteId);
  // @formatter:on
