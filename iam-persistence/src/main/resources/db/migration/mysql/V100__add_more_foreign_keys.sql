...
DELETE FROM device_code WHERE auth_holder_id NOT IN (SELECT id FROM authentication_holder);
ALTER TABLE device_code MODIFY COLUMN auth_holder_id BIGINT(20) NOT NULL;
ALTER TABLE device_code ADD CONSTRAINT FK_device_code_auth_holder_id FOREIGN KEY (auth_holder_id) REFERENCES authentication_holder (id) ON DELETE CASCADE;
